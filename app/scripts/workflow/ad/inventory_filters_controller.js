    ...
    angularAMD.controller('InventoryFiltersController', function ($scope, workflowService, fileReader, Upload) {
                    ...
                    .then(function (result) {
                        ...
                        if ($scope.mode === 'edit') {
                            ...
                            if ($scope.savedDomainListIds) {
                                $scope.savedDomainListIds = $scope.savedDomainListIds
                                    .concat($scope.getAd_result.targets.appTargets.inheritedAppList.ADVERTISER);
                            } else {
                                $scope.savedDomainListIds =
                                    $scope.getAd_result.targets.appTargets.inheritedAppList.ADVERTISER;
                            }
                            ...
                            if ($scope.savedDomainListIds && $scope.savedDomainListIds.length) {
                                selectedLists = _.map($scope.workflowData.inventoryData,
                                    function (value) {
                                        return _.contains($scope.savedDomainListIds,
        ...
        $scope.selectDomainList = function (event) {
            ...
            function process(selectedLists) {
                ...
                if (element.attr('checked') !== 'checked') {
                    ...
                    currentDomainList =
                        _.filter($scope.workflowData.inventoryData, function (domainList) {
        ...
        $scope.workflowData.inventoryStateChanged = function (event, inventoryObj) {
            ...
            if (selectedChkBox.length > 0) {
                idx =
                    _.findIndex($scope.workflowData.selectedLists, function (item) {
