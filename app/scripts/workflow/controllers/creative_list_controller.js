        ...
        angularAMD.controller('CreativeListController', function ($scope, $rootScope, $routeParams, $route, $location,
            ...
            $scope.creativeSearchFunc = function () {
                var searchVal = $scope.creativeSearch,
                    //selectedClientObj, // NOTE: Not used
                    qryStr = '';
            ...
            $scope.updateCreative = function () {
                var putCrDataObj = {
                    name: $scope.selectedCreativeData.name,
                    tag: $scope.editableTag,
                    sizeId: $scope.selectedCreativeData.size.id,
                    creativeFormat: $scope.selectedCreativeData.creativeFormat,
                    creativeType: $scope.selectedCreativeData.creativeType,
                    sslEnable: 'true',
                    updatedAt: $scope.selectedCreativeData.updatedAt
                };
            ...
            $(function () {
                $(window).scroll(function () {
                    ...
                    if ($(window).scrollTop() + $(window).height() === $(document).height() && !isSearch) {
                        ...
                        //console.log('$(this).scrollTop():'+$(this).scrollTop()+'$(this).innerHeight():'+
                        // $(this).innerHeight()+'$(this)[0].scrollHeight:'+$(this)[0].scrollHeight)
                        var selectedClientObj = localStorage.selectedClient && JSON.parse(localStorage.selectedClient);

                        if (selectedClientObj && (window.location.href.indexOf('creative/list') > -1)) {
                            creativeList.getCreativesList(JSON.parse(localStorage.selectedClient).id, '', '',
                                $scope.pageSize, $scope.pageNo);
