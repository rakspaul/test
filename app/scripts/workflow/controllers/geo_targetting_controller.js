define(['angularAMD', 'common/services/constants_service', 'workflow/services/workflow_service', // jshint ignore:line
    'common/services/zip_code', 'lrInfiniteScroll'], function (angularAMD) {
    angularAMD.controller('GeoTargettingController', function ($scope, $rootScope, $timeout, $filter, constants,
        ...
        var DATA_MAX_SIZE = 200,
            ...
            geoTargeting = {
                ...
                regionWrapper: function (geoItem, idx) {
                    _.each(geoItem.data, function (item) { // jshint ignore:line
                        ...
                        pos = _.findIndex($scope.geoSelectedItems[idx].countries, function (obj) { // jshint ignore:line
                ...
                countryRegionWrapper: function (geoItem, idx) {
                    _.each(geoItem.data, function (item) { // jshint ignore:line
                        ...
                        if (item.geoType === 'REGION') {
                            pos = _.findIndex( // jshint ignore:line
                                $scope.geoSelectedItems[idx].countries,
                                function (obj) {
                ...
                regionCityWrapper: function (geoItem, idx) {
                    _.each(geoItem.data, function (item) { // jshint ignore:line
                        ...
                        if (item.geoType === 'REGION') {
                            ...
                            pos =
                                _.findIndex($scope.geoSelectedItems[idx].countries, function (obj) { // jshint ignore:line
                                    return item.countryCode === obj.countryCode;
                                });
                            ...
                            pos1 =
                                _.findIndex( // jshint ignore:line
                                    $scope.geoSelectedItems[idx].countries[countryLen].regions,
                                    function (obj) {
                                        return item.countryCode === obj.countryCode;
                                    });
                        ...
                        if (item.geoType === 'CITY') {
                            ...
                            if (pos1 < 0) {
                                ...
                                // can not be negative. WHO COMMENTED THIS???
                                // countryLen = pos1;
                                $scope.geoSelectedItems[idx].countries[countryLen].regions.push(item.parent);
                                delete item.parent;
                            }
                ...
                showToolTip: function (elem) {
                    ...
                    var tooltip_left = elem.offset().left - elem.closest('.btn-group').offset().left,
                        tooltip_width = elem.closest('.btn-group').find('.common_tooltip').width(),
                ...
                updateSelectedGeoList : function (isChecked, type) {
                    ...
                    if (type === 'countries'  || type === 'regions'  || type === 'cities' ) {
                        ...
                        // if selected tab is regions and country is not selected and we change
                        // the include/exclude toggle
            ...
            countriesWrapper = {
                setData : function (bool, data,  isIncluded) {
                    var countryData = $.extend(true, [], data); // jshint ignore:line
            ...
            regionsWrapper = {
                ...
                list: function (requestType) {
                    ...
                    // fetch regions for those countries
            ...
            citiesWrapper = {
                ...
                list: function (requestType) {
                    ...
                    if (selectedRegions.length > 0) {
                        ...
                        // if countries are selected and regions is not selected, pick the country codes of
                        // selected countries. fetch cities fot those countries
                        selectedCountries = $scope.geoData.countries.selected;
        ...
        $scope.saveGeography = function (cancelClicked) {
            ...
            if ($scope.geoData.zip.selected.length === 0 &&
                geoTargetsData && geoTargetsData.targets &&
            ...
            if (type === 'zip') {
                ...
                if (type === 'countries') {
                    ...
                    _.each($scope.geoSelectedItems, function (obj) { // jshint ignore:line
                        obj.countries =
                            _.filter(obj.countries, function (obj) { // jshint ignore:line
                ...
                if (type === 'regions') {
                    ...
                    _.each($scope.geoSelectedItems, function (obj) { // jshint ignore:line
                        _.each(obj.countries, function (country) { // jshint ignore:line
                            country.regions =
                                _.filter(country.regions, function (obj) { // jshint ignore:line
                                    return obj.id !== item.id;
                                });
                        });
                    });
                        ...
                        _.each(obj.countries, function (country) { // jshint ignore:line
                            _.each(country.regions, function (region) { // jshint ignore:line
                                _.each(region.cities, function (city, idx) { // jshint ignore:line
                                    region.cities[idx] =
                                        _.filter(city, function (obj) { // jshint ignore:line
                                            return obj.id !== item.id;
                                        });
                                });
                            });
        ...
        $scope.addZipCode = function (obj) {
            ...
            _.each(zipCodesObj.duplicate, function (removeval) { // jshint ignore:line
                _.each(zipCodeList, function (obj, idx) { // jshint ignore:line
                    if (obj) {
                        _.each(obj.added, function (val) { // jshint ignore:line
                            if (removeval === val) {
