                    ...
                    workflowService
                        ...
                        .then(function (result) {
                            if (result.status === 'OK' || result.status === 'success') {
                                ...
                                _.each($scope.selectedCampaign.pixelList, function (item) { // jshint ignore:line
                                    item.createdAt = momentService.newMoment(item.createdAt).format('YYYY-MM-DD');
                                });

                                if (pixels && pixels.length > 0) {
                                    _.each(pixels, function (id) { // jshint ignore:line
                                        var _pixel =
                                            _.filter($scope.selectedCampaign.pixelList, // jshint ignore:line
                                                function (obj) {
                                                    return obj.id === id;
                                                })[0];
            ...
            $scope.$parent.selectPixel = function (pixel) {
                if (pixel) {
                    ...
                    if (pixelIndex === -1) {
                        ...
                        index =
                            _.findIndex($scope.selectedCampaign.pixelList, function (list) { // jshint ignore:line
