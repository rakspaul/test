    ...
    function (angularAMD) {
        ...
        angularAMD.controller('CustomReportController', function ($routeParams, $rootScope, $scope, $route, $window,
            ...
            $scope.reports.reportDefinition.timeframe.start_date =
                moment().subtract(1, 'day').format(constants.DATE_UTC_SHORT_FORMAT); // jshint ignore:line
            ...
            $scope.reports.schedule.startDate =
                moment().subtract(0, 'days').format(constants.DATE_UTC_SHORT_FORMAT); // jshint ignore:line
            ...
            $scope.initializeMetrics = function (dataObj, selectedDim) {
                ...
                if (metricsData.quality_metrics && metricsData.quality_metrics.display_metrics !== undefined) {
                    $scope.qualityMetricsData = (metricsData.quality_metrics.display_metrics)
                        .concat(metricsData.quality_metrics.video_metrics);
                }

                $scope.qualityMetrics = [];

                _.each($scope.qualityMetricsData, function (key) { // jshint ignore:line
                ...
                $scope.totalMetrics = $scope.totalDeliveryMetrics +
                    $scope.totalCostMetrics +
                    $scope.totalVideoMetrics +
                    $scope.totalQualityMetrics +
                    $scope.totalPacingMetrics;
            };
            ...
            _customctrl.getDimensionList = function (data, selectedMetrics) {
                ...
                if ($scope.selectedMetricsList.length < $scope.totalMetrics) {
                    ...
                    if (metricsCategorizedKey.quality_metrics.display_metrics !== undefined) {
                        metricsCategorizedKey.quality_metrics =
                            (metricsCategorizedKey.quality_metrics.display_metrics).
                                concat(metricsCategorizedKey.quality_metrics.video_metrics);
                    }
                    ...
                    if (arr.quality_metrics && arr.quality_metrics.display_metrics !== undefined) {
                        arr.quality_metrics = (arr.quality_metrics.display_metrics)
                            .concat(arr.quality_metrics.video_metrics);
                    }
            ...
            _customctrl.getDataBasedOnTabSelected = function (activeTab, typeofDimension, currIdx) {
                ...
                _.each(data, function (d, index) { // jshint ignore:line
                    ...
                    if (activeTab === 'quality_metrics') {
                        tabData =
                            _.extend({}, d.quality_data.display_data, d.quality_data.video_data); // jshint ignore:line
                    } else {
                        tabData = d[activeTab];
                    }
                        ...
                        if (mkey !== 'dimension') {
                            _.each(d[mkey], function (value, key) { // jshint ignore:line
                                if (mkey === 'quality_data') {
                                    _.each(value,function (v,k) { // jshint ignore:line
                                        found = false;
            ...
            _customctrl.createRequestParams = function (filterText, offset, isPrimary, rowIndex_2D,dataFormat) {
                ...
                if ($scope.reports.reportDefinition.dimensions[filterDataKey].dimension) {
                    ...
                    str += ((dataFormat && dataFormat==='csv') ?
                            '&second_dim=' : '&first_dim_filter=') +
            ...
            _customctrl.getReportData = function () {
                _customctrl.fetchReportData($scope.selectedMetricsList,
                    _customctrl.createRequestParams(null, $scope.firstDimensionoffset, 1), null, function (respData) {
            ...
            _customctrl.resetReportDataCnt = function () {
                ...
                _.each(metricsInTabOrder,function (mTab) { // jshint ignore:line
                    if (($scope[mTab+'Metrics'].minOneSelected) && !firstTabDetected) {
                        ...
                        $('.custom_report_response_page .custom_report_response_table ' +
                            '.custom_report_scroll .heading_row').css('left','0');
                    }
                });
            };
                    ...
                    _.each(newJSON.dimensions,function (item) { // jshint ignore:line
                        if (item.type === 'Primary') {
                            ...
                            if (oldJSON.dimensions.primary.dimension !== item.dimension ||
                                oldJSON.dimensions.primary.value &&
                                oldJSON.dimensions.primary.value !==
                                $scope.reports.reportDefinition.dimensions.primary.value) {
                                retVal = true;
                            }
                    ...
                    return {
                        mediaplan: function (dimension, curRowIndx, curSecDimIndx) {
                                ...
                                if (dimension === 'first_dimension') {
                                    ...
                                    mediaPlanId =
                                        $scope.reportMetaData[dimension][curRowIndx][curSecDimIndx].dimension.id;
                                }
                        ...
                        ad: function (dimension, curRowIndx, curSecDimIndx) {
                            ...
                            if ((adGroupId !== -1) &&
                                ...
                                $location.url('mediaplan/' + mediaPlanId + '/lineItem/' + lineItemId +
                        ...
                        routeToLink: function (ev,dimension,curSecDimIndx) {
                            ...
                            if (dimension === 'first_dimension') {
                                ...
                                metricObj =
                                    $scope.metricValues[dimension][currentRowIndex][$scope.activeTab][curSecDimIndx];
                            }

                            if ((metricObj instanceof Object) &&
                                ((metricObj.type === 'campaign_name') ||
                                (metricObj.type === 'ad_name'))) {
            ...
            $scope.showDataForClikedDimension = function (ev, value, rowIndex, loadMore) {
                ...
                if (!currFirtDimensionElem.hasClass('treeOpen') &&
                    ...
                    $scope.createData().reportDefinition) && $scope.isReportForMultiDimension !== false) {
                ...
                if (!currFirtDimensionElem.hasClass('treeOpen') || loadMore) {
                    ...
                    _customctrl.fetchReportData($scope.selectedMetricsList, paramsObj, currentRowIndex,
                        function (respData, currentRowIndex) {
                            // TODO: (Lalding 11th July 2016) Is the comma expression below intentional?
                            if ($scope.activeTab, $scope.secDimensionLoadIcon[$scope.activeTab] !== undefined &&
                                ...
                                if (respData) {
                                    ...
                                    if (resultLen > 0) {
                                        ...
                                        if (_customctrl.reportPageNum_2D[$scope.activeTab][currentRowIndex] === 1) {
                                            $scope.secondDimensionReportDataNotFound
                                                [$scope.activeTab][currentRowIndex] = true;
            ...
            $(document).ready(function () {
                ...
                $('.input-daterange')
                    ...
                    .on('changeDate', function () {
                        var frequencyDropDown = $('.frequency').text().trim(),
                            ...
                            endDateChecker = new Date(startOn.val()),
                            startDateCheckerRange = startOn.val(),
                            endDateCheckerRange = startOn.val(),
                            theDateDifference =
                ...
                $scope.setAllMetrics = function () {
                    if ($scope.deliveryMetrics.isAllSelected &&
                        ($scope.costMetrics.isAllSelected ||
                        !$scope.showCost) &&
                        $scope.videoMetrics.isAllSelected &&
                        ($scope.qualityMetrics.isAllSelected ||
                        !$scope.showQuality)) {
                ...
                $scope.saveMetrics = function () {
                    ...
                    _.each(metricVarKeys,function (eachMetric) { // jshint ignore:line
                        var metricArray = [],
                            nameOfMetric = eachMetric.split(/(?=[A-Z])/)[0];
                ...
                $scope.validateScheduleDate = function () {
                    ...
                    currDate = momentService.todayDate('YYYY-MM-DD');
                ...
                $scope.prefillData = function (reportData) {
                    ...
                    _.each(metricVarKeys, function (eachMetric) { // jshint ignore:line
                        // eg: metricType = pacingMetrics,
                        // below 2 lines extracts only pacing and captailize first letter eg: Pacing
                ...
                getCustomReportMetrics = function () {
                    dataService
                        ...
                        .then(function (result) {
                            ...
                            modifiedDimensionArr = result.data.data[0];
                                ...
                                dataService
                                    ...
                                    .then(function (response) {
                                        if (response.status === 'success') {
                                            ...
                                            $('.img_table_txt').html('Please select dimensions, timeframe and any ' +
                                                'additional <br> parameters to update the report');
                ...
                metricsTabIdTab.forEach(function (id) {
                    $('#'+id+' .custom_report_scroll').scroll(function () {
                        $('.custom_report_response_page .custom_report_response_table ' +
                                '.custom_report_scroll .heading_row')
                            .css({left: '-' + $('#'+id+' .custom_report_scroll').scrollLeft() + 'px'});
