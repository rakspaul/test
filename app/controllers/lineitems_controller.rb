class LineitemsController < ApplicationController
  ...
  def index
    ...
    @order = Order.includes(:lineitems => [  { :creatives => [ :lineitem_assignment, :ad_assignments ] } ]).order('CAST(io_lineitems.alt_ad_id AS INTEGER) ASC, lineitem_assignments.start_date ASC, creatives.size ASC').find(params[:order_id])
    ...
    if @lineitems.empty? && !@ads.empty?
      ...
      @ads.group_by(&:alt_ad_id).each do |alt_ad_id, ads|
        ...
        ads.each do |ad|
          ...
          ad.audience_groups.each{|ag| li.audience_groups << ag}
