class BlockSitesExport
  ...
  def separate_advs_and_groups
    ...
    @blocked_sites.each do |block_site|
      ...
      if block_site.visited == false
        ...
        @blocked_adv_and_grp << adv_and_grp_hash
      end
    end
  end
  ...
  def write_to_excel
    ...
    sheet = book.create_worksheet :name => "Blocked Advertiser and Groups"
    row_no = 0
    col_no = 0

    @blocked_adv_and_grp.each do |bs|
      row_count = row_no + 2
