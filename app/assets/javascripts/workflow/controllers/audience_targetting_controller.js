    ...
    'use strict';
        angObj.controller('AudienceTargettingController', function ($scope,  audienceService, $rootScope, workflowService) {
            ...
            $scope.fetchAllAudience = function (loadMoreFlag) {
                ...
                audienceService.fetchAudience($scope.sortColumn, $scope.sortOrder,$scope.pageNumber,
                    $scope.pageSize, $scope.selectedKeywords,$scope.selectedSource,$scope.selectedCategory)
            ...
            function processAudienceEdit() {
                ...
                if (previouslySelectedAudience && previouslySelectedAudience.length > 0) {
                    for (i = 0; i < previouslySelectedAudience.length; i++) {
                        ...
                        // cant call $scope.selectAudience($scope.audienceList[index]) because this will toggle
                        // selection when filter is clicked
                        if (index !== -1) {
