...
(function () {
    ...
    angObj.controller('CampaignOverViewController',
        function ($scope, $rootScope, $window, $routeParams, constants, workflowService, $timeout, $location,
                  utils, momentService) {
            $('.main_navigation')
                .find('.active')
                .removeClass('active')
                ...
                getCampaignData: function (campaignId) {
                    workflowService.getCampaignData(campaignId).then(function (result) {
                        if (result.status === 'OK' || result.status === 'success') {
                            ...
                            //set campaign start date as lower limit startDate
                            $scope.setStartdateIndependant =
                            ...
                            //console.log(campaignStartTime);
                           // var campaignStartTime = moment($scope.workflowData['campaignData'].startTime).format(constants.DATE_US_FORMAT);
                            // console.log(campaignStartTime);
                            ...
                            //var campaignEndTime = moment($scope.workflowData['campaignData'].endTime).format(constants.DATE_US_FORMAT);
                            // console.log(campaignEndTime);
            ...
            $scope.extractor = function (IndividualAdsData) {
                ...
                //console.log($scope.setStartdateIndependant);
                ...
                if (ascending.length > 0) {
                    ...
                    //console.log(moment(ascending[0].startTime).format('YYYY-MM-DD HH:mm:ss.SSS'));
                    //console.log(moment($scope.lowestStartTime).format('YYYY-MM-DD HH:mm:ss.SSS'));
                    // $scope.lowestStartTime = moment(ascending[0].startTime).format(constants.DATE_US_FORMAT);
                ...
                descending = _.sortBy(endDateHigh, function (o) {//method to find the highest endTime
                    return o.endTime;
                });
                descending.reverse();
                ...
                if (descending.length > 0) {
                    ...
                    //console.log(moment(descending[0].endTime).format('YYYY-MM-DD HH:mm:ss.SSS'));
                    //console.log(moment($scope.highestEndTime).format('YYYY-MM-DD HH:mm:ss.SSS'));
                    // $scope.highestEndTime = moment(descending[0].endTime).format(constants.DATE_US_FORMAT);
            ...
            $scope.editAdforAdGroup = function(campaignId, stTime, edTime, adsId, groupId) {
                ...
                path = '/mediaplan/' + campaignId + '/ads/' + adsId + '/edit';
                if (groupId && adsId) {
    ...
    angObj.controller('GetAdgroupsController',
        function ($scope, $window, $routeParams, constants, workflowService, $timeout, utils, $location) {
            ...
            // TODO: This method is redundant. It's never used!
            $scope.convertEST = function (date, format) {
                return utils.convertToEST(date,format);
            };
        });

    angObj.controller('CreateAdGroupsController',
        function ($scope, $window, $routeParams, constants, workflowService, $timeout, utils, $location) {
