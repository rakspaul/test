...
ReachUI.Reports.Dimensions = function() {
	...
	var Report = Backbone.Model.extend({
      ...
      if (this.getDimensionLength() < 1) {
        this.get("dimensions").reset();
         this.get("dimensions").trigger("change");
        this.deleteDefaultColumns();
      }
		},
      ...
      this.set({columns: columns})
		...
		getReportTableColumns: function() {
			...
			this.getDimensions().each(function(dimension){
			 	tableColumns.push({field_name:dimension.getFieldName(), field_header: dimension.getDisplayName()});
			});

			tableColumns = tableColumns.concat(this.getColumns())
			return tableColumns;
		},

    setDefaultColumns: function() {
      this.setColumns([
                        {field_name:'impressions',field_header: 'Impressions'}, 
                        {field_name:'clicks',field_header: 'Clicks'}, 
                        {field_name:'ctr',field_header: 'CTR %'}
                      ]);
    },

    deleteDefaultColumns: function() {
      this.setColumns([]);
    },

		getReportTableColumnFields: function() {
			var tableColumns = [],
				columnNames = [];

			this.getDimensions().each(function(dimension){
				tableColumns.push(dimension.getFieldName());
			});

			columnNames = this.getColumns();
			for (var key in columnNames ){
				tableColumns.push(columnNames[key].field_name);
			}
		...
		getQueryString: function() {
			...
			str += "&cols=" + this.getReportTableColumnFields();
	...
	var ReportDataItem = Backbone.Model.extend({});
    ...
    className: "table table-striped table-bordered",
		...
		initialize: function(){
			...
			this.model.on("change:report_data", this.onReportDataReset, this);
		},
	...
	var getReportDataFromServer = function() {
		var request = $.ajax({
			dataType: "json",
		...
		request.success(function(data){
			report.setReportData(new ReportData(data.records));
		});
